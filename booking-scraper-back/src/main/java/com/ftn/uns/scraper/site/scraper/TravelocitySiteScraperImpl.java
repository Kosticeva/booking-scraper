...
public class TravelocitySiteScraperImpl implements SiteScraper {
    ...
    public Results scrapePage(SearchQuery query) {
        ...
        for(SearchMarker marker: query.getMarkers()){
            if(marker.getType() == SiteType.TRAVELOCITY){
        ...
        if(travelocityMarker == null) {
            ...
            travelocityMarker.setType(SiteType.TRAVELOCITY);
        }
        ...
        while(results.getHotels().size() < 10){
           ...
           @Cleanup BufferedWriter wr = new BufferedWriter(new FileWriter(new File("src/main/resources/trav.html")));
            wr.write(page.asXml());
            ...
            for(HtmlElement hotel: hotels){
                ...
                result.setType(SiteType.TRAVELOCITY);
